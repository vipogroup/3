...
describe("Stage 1 - Base: Products", () => {
  it("GET /api/products returns 200 and array", async () => {
    ...
    record("api.products.list", ok, `status=${res.status}; length=${Array.isArray(res.body)?res.body.length:"n/a"}`, "Implement GET /api/products to return an array");
  });

  it("GET /products page loads", async () => {
    const res = await request(base).get("/products");
    const ok = res.status === 200;
    record("ui.products.page", ok, `status=${res.status}`, "Ensure /products route renders without auth");
  });
});
