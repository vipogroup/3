...
function writeReport() { try { const outDir = path.join(process.cwd(), "reports", "tests"); fs.mkdirSync(outDir, { recursive: true }); fs.writeFileSync(path.join(outDir, "stage8.json"), JSON.stringify(results, null, 2)); } catch {} }

function randPhone() { return "05" + Math.floor(10000000 + Math.random()*89999999).toString(); }

async function register(role) {
  const phone = randPhone();
  const res = await request(base).post("/api/auth/register").send({ fullName: `${role.toUpperCase()} REF`, phone, role, password: "12345678A!" });
...
describe("Stage 8 - Referral / חבר-מביא-חבר", () => {
  it("join?ref=XYZ sets cookie; creating sale by another agent sets refAgentId and commissionReferral", async () => {
    ...
    record("ref.join.cookie", joinOk, `status=${joinRes.status}`, "Implement /join?ref=XYZ to set refSource cookie");

    // Create sale by agent2 with ref cookie present
    const saleRes = await request(base)
      .post("/api/sales")
      .set("Cookie", [ l2.cookie, `refSource=${refCode}` ])
      .send({ productId: "656565656565656565656565", customerName: "Ref Buyer", customerPhone: "0550003333", salePrice: 1000 });

    const hasReferral = saleRes.body && ("refAgentId" in saleRes.body) && ("commissionReferral" in saleRes.body);
    record("ref.sale.fields", hasReferral, `status=${saleRes.status}`, "On sale create, set refAgentId and commissionReferral = price*0.02 when refSource present");
