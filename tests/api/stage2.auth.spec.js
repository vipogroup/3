...
function writeReport() { try { const outDir = path.join(process.cwd(), "reports", "tests"); fs.mkdirSync(outDir, { recursive: true }); fs.writeFileSync(path.join(outDir, "stage2.json"), JSON.stringify(results, null, 2)); } catch {} }
...
describe("Stage 2 - Auth + Roles", () => {
  it("register admin and agent (201)", async () => {
    ...
    record("auth.register", ok, `admin=${a.res.status}; agent=${g.res.status}`, "POST /api/auth/register should accept fullName, phone, role, password");
  });
  ...
  it("login sets httpOnly JWT cookie; wrong password => 401", async () => {
    ...
    const pass = ok.status === 200 && setCookie.includes("token=") && setCookie.toLowerCase().includes("httponly") && bad.status === 401;
    record("auth.login.cookie", pass, `ok=${ok.status}; bad=${bad.status}; cookie=${setCookie}` , "Login must set httpOnly cookie 'token' and reject wrong password");
  });
});
