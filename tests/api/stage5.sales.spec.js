...
function writeReport() {
  try {
    ...
    fs.writeFileSync(path.join(outDir, 'stage5.json'), JSON.stringify(results, null, 2));
  } catch {}
}
...
async function register(role) {
  ...
  const res = await request(base)
    ...
    .send({ fullName: `${role.toUpperCase()} SALES`, phone, role, password: '12345678A!' });
...
describe('Stage 5 - Sales + Commission + Status', () => {
  it('agent creates sale (commission 5%); admin can change status; agent cannot', async () => {
    ...
    record(
      'sales.reg.users',
      regOk,
      `agent=${agent.res.status}; admin=${admin.res.status}`,
      'Register agent/admin via /api/auth/register',
    );
    ...
    const create = await request(base)
      .post('/api/sales')
      .set('Cookie', agentLogin.cookie)
      .send({ productId, customerName: 'Test Buyer', customerPhone: '0550001111', salePrice });

    const createOk = create.status === 201;
    const commission = create.body?.commission;
    const commissionOk =
      typeof commission === 'number' && Math.abs(commission - salePrice * 0.05) < 1e-6;
    const statusOk = create.body?.status === 'pending';
    record(
      'sales.create.agent',
      createOk && commissionOk && statusOk,
      `status=${create.status}; commission=${commission}; status=${create.body?.status}`,
      "POST /api/sales should compute 5% and set status 'pending'",
    );
    ...
    if (id) {
      // Admin change status
      const updAdmin = await request(base)
        .put(`/api/sales/${id}`)
      ...
      record(
        'sales.status.admin',
        updAdmin.status === 200,
        `status=${updAdmin.status}`,
        'Admin should be able to change sale status',
      );

      // Agent cannot change status (expected 403)
      const updAgent = await request(base)
        .put(`/api/sales/${id}`)
        .set('Cookie', agentLogin.cookie)
        .send({ status: 'completed' });
      record(
        'sales.status.agent.forbidden',
        updAgent.status === 403,
        `status=${updAgent.status}`,
        'Agent should not be able to change sale status',
      );

      // Agent list only his sales
      const list = await request(base).get('/api/sales').set('Cookie', agentLogin.cookie);
      record(
        'sales.list.agent.scope',
        list.status === 200 && Array.isArray(list.body),
        `status=${list.status}`,
        'Agent should see only his sales',
      );
    }
  });
});
