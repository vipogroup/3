...
function writeReport() {
  try {
    ...
    fs.writeFileSync(path.join(outDir, 'stage7.json'), JSON.stringify(results, null, 2));
  } catch {}
}
...
async function register(role) {
  ...
  const res = await request(base)
    ...
    .send({ fullName: `${role.toUpperCase()} NOTIFY`, phone, role, password: '12345678A!' });
...
function clearLog() {
  try {
    fs.rmSync(path.join(process.cwd(), 'logs', 'notifications.log'), { force: true });
  } catch {}
}
