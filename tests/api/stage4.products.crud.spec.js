...
function writeReport() {
  try {
    ...
    fs.writeFileSync(path.join(outDir, 'stage4.json'), JSON.stringify(results, null, 2));
  } catch {}
}
...
describe('Stage 4 - Products CRUD + Upload', () => {
  it('admin can POST/GET/PUT/DELETE; agent 403 on write ops', async () => {
    ...
    const createByAgent = await request(base)
      .post('/api/products')
      .set('Cookie', agentLogin.cookie)
      .send(newProd);
    record(
      'products.post.agent.forbidden',
      createByAgent.status === 403,
      `status=${createByAgent.status}`,
      'Enforce role guard: only admin can POST /api/products',
    );
    ...
    record(
      ...
      `status=${list.status}`,
      'GET /api/products must return array',
    );

    if (prodId) {
      const update = await request(base)
      ...
      const delByAgent = await request(base)
        .delete(`/api/products/${prodId}`)
        .set('Cookie', agentLogin.cookie);
      record(
        'products.delete.agent.forbidden',
        delByAgent.status === 403,
        `status=${delByAgent.status}`,
