...
async function uiLogin(page: any, phone: string, password: string) {
  ...
  await Promise.all([page.waitForURL(/\/(agent|admin|)/), page.click('button[type="submit"]')]);
}
...
function record(stage: number, checks: any[]) {
  try {
    ...
    fs.writeFileSync(
      path.join(outDir, `stage${stage}.json`),
...
test.describe('Stage 3 - Layout + Routing + Role-Guard', () => {
  ...
  test('agent sees limited menu; admin sees full; agent blocked from /admin', async ({
    page,
    request,
  }) => {
    ...
    const agentMenuTexts = await page
      .locator('nav >> li')
      .allInnerTexts()
      .catch(() => [] as string[]);
    const agentJoined = agentMenuTexts.join(' | ');
    const agentHasLimited =
      /(My Sales|Sales)/.test(agentJoined) &&
      /(New Sale)/.test(agentJoined) &&
      !/Products/.test(agentJoined) &&
      !/Reports/.test(agentJoined);
    checks.push({
      name: 'agent.menu',
      passed: agentHasLimited,
      evidence: agentJoined,
      hint: 'Agent should not see Products/Reports',
    });
    ...
    checks.push({
      name: 'agent.admin.guard',
      passed: redirected,
      evidence: url,
      hint: 'Agent must be redirected away from /admin',
    });

    // Admin login and menu assertions
    await page.context().clearCookies();
    await uiLogin(page, adminPhone, '12345678A!');
    const adminMenuTexts = await page
      .locator('nav >> li')
      .allInnerTexts()
      .catch(() => [] as string[]);
    const adminJoined = adminMenuTexts.join(' | ');
    const adminHasFull =
      /Products/.test(adminJoined) && /Reports/.test(adminJoined) && /Agents/.test(adminJoined);
    checks.push({
      name: 'admin.menu',
      passed: adminHasFull,
      evidence: adminJoined,
      hint: 'Admin should see full menu',
    });
  });
