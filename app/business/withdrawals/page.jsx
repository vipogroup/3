...
export default function BusinessWithdrawalsPage() {
  ...
  const fetchWithdrawals = useCallback(
    async (targetPage, statusValue, searchValue) => {
      ...
      try {
        ...
        console.error('BUSINESS_WITHDRAWALS_FETCH_ERROR', err);
  ...
  }, [page, statusFilter, debouncedSearch, fetchWithdrawals, authorized]);
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">ניהול בקשות משיכה מסוכני העסק</p>
          </div>
...
function WithdrawalActionModal({ open, withdrawalId, onClose, onActionComplete }) {
  ...
  const loadDetails = useCallback(async () => {
    ...
    try {
      ...
      console.error('BUSINESS_WITHDRAWAL_DETAILS_ERROR', err);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      console.error('BUSINESS_WITHDRAWAL_UPDATE_ERROR', err);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
                ...
                <p className="mt-2 text-lg font-bold" style={{ color: 'var(--primary)' }}>
                  {formatCurrencyILS(withdrawal.amount)}
                </p>
              </div>
              <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
