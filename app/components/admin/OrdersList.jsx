...
export default function OrdersList() {
  ...
  return (
    <div>
      ...
      <div className="hidden md:block bg-white rounded-xl shadow-lg overflow-hidden" style={{
        border: '2px solid transparent',
        backgroundImage: 'linear-gradient(white, white), linear-gradient(135deg, #1e3a8a, #0891b2)',
        backgroundOrigin: 'border-box',
        backgroundClip: 'padding-box, border-box'
        ...
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: '2px solid #0891b2' }}>
              <th className="px-6 py-3 text-right text-xs font-medium uppercase" style={{ color: '#1e3a8a' }}>מזהה</th>
              <th className="px-6 py-3 text-right text-xs font-medium uppercase" style={{ color: '#1e3a8a' }}>לקוח</th>
              <th className="px-6 py-3 text-right text-xs font-medium uppercase" style={{ color: '#1e3a8a' }}>סכום</th>
              <th className="px-6 py-3 text-right text-xs font-medium uppercase" style={{ color: '#1e3a8a' }}>סטטוס</th>
              <th className="px-6 py-3 text-right text-xs font-medium uppercase" style={{ color: '#1e3a8a' }}>תאריך</th>
            ...
            {filteredOrders.map((order) => {
              ...
              return (
                <tr key={order._id} className="border-b border-gray-100 transition-all" onMouseEnter={(e) => e.currentTarget.style.background = 'linear-gradient(135deg, rgba(30, 58, 138, 0.02) 0%, rgba(8, 145, 178, 0.02) 100%)'} onMouseLeave={(e) => e.currentTarget.style.background = 'white'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap font-bold" style={{ color: '#1e3a8a' }}>
      ...
      <div className="md:hidden space-y-4">
        {filteredOrders.map((order) => {
          ...
          return (
            <div key={order._id} className="bg-white rounded-xl p-4 shadow-lg" style={{
              border: '2px solid transparent',
              backgroundImage: 'linear-gradient(white, white), linear-gradient(135deg, #1e3a8a, #0891b2)',
              backgroundOrigin: 'border-box',
              backgroundClip: 'padding-box, border-box'
              ...
              <div className="space-y-2 mb-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm">{new Date(order.createdAt).toLocaleDateString("he-IL")}</p>
                  </div>
                </div>
              </div>
        ...
        {filteredOrders.length === 0 && (
          <div className="bg-white rounded-xl p-8 text-center text-gray-500 shadow-lg" style={{
            border: '2px solid transparent',
            backgroundImage: 'linear-gradient(white, white), linear-gradient(135deg, #1e3a8a, #0891b2)',
            backgroundOrigin: 'border-box',
            backgroundClip: 'padding-box, border-box'
