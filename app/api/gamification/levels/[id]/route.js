...
async function PUTHandler(req, { params }) {
  try {
    ...
    if (typeof body.minSalesAmount === 'number' && body.minSalesAmount >= 0)
      update.minSalesAmount = body.minSalesAmount;
    if (typeof body.minDeals === 'number' && body.minDeals >= 0) update.minDeals = body.minDeals;
    if (typeof body.commissionBoostPct === 'number' && body.commissionBoostPct >= 0)
      update.commissionBoostPct = body.commissionBoostPct;
    if (body.badgeColor && ['bronze', 'silver', 'gold', 'platinum'].includes(body.badgeColor))
      update.badgeColor = body.badgeColor;
