...
export async function GET(request, { params }){
  try {
    ...
    const size = Math.min(Math.max(parseInt(urlObj.searchParams.get("size") || "340", 10), 128), 1024);
    ...
    const buf = await QRCode.toBuffer(url, { type:"png", width:size, margin:1, errorCorrectionLevel: withLogo? "H":"M", color:{ dark:"#000000", light:"#ffffff" } });
