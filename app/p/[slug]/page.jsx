  ...
  return (
    <main className="grid gap-6 lg:grid-cols-3">
      ...
      <aside className="card h-fit">
        ...
        {!isGroup && (
          <Link className="btn w-full" href={`/checkout?slug=${p.slug}`}>
            הזמנה מאובטחת
          </Link>
        )}
        {isGroup && (
          <Link className="btn w-full" href={`/group-join?slug=${p.slug}`}>
            מצטרף עכשיו (דמי רצינות)
          </Link>
        )}

        <div className="mt-4">
          <a className="btn w-full" href="https://wa.me/0587009938" target="_blank">
            דברו איתנו בוואטסאפ
          </a>
        </div>
      </aside>
