...
async function initClient() {
  ...
  if (!mongoConfig.uri || mongoConfig.uri === '') {
    console.log('[SKIP] Skipping MongoDB connection - no URI configured');
  ...
  if (!cached.promise) {
    if (!hasLoggedConnectAttempt) {
      ...
      console.log('[PIN] URI:', obfuscateUri(mongoConfig.uri));
    }
      ...
      .catch((err) => {
        ...
        if (!hasLoggedConnectError) {
          ...
          if (err.message?.includes('authentication failed') || err.code === 18) {
            console.warn('[WARN] MongoDB auth failed - continuing without DB');
            cached.client = null;
            cached.db = null;
            return cached;
          }
