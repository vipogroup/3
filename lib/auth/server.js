...
import { hasPermission, isSuperAdminUser } from '@/lib/superAdmins';
...
function normalizeUser(payload) {
  ...
  return {
    ...
    tenantId: base.tenantId || payload.tenantId || null,
    impersonating: base.impersonating || payload.impersonating || false,
    originalRole: base.originalRole || payload.originalRole || null,
    raw: base,
  };
}
...
async function hydrateUserFromDb(user) {
  ...
  try {
    ...
    return {
      ...user,
      role: user.impersonating ? user.role : (doc.role || user.role), // Keep impersonation role
      email: doc.email || user.email,
      fullName: doc.fullName || user.fullName,
      phone: doc.phone || user.phone,
      showPushButtons: doc.showPushButtons,
      // When impersonating, use tenantId from JWT; otherwise use from DB
      tenantId: user.impersonating ? user.tenantId : (doc.tenantId || null),
      raw: { ...(user.raw || {}), db: doc },
    };
  } catch (err) {
...
/**
 * For Server Components - require specific admin permission (with optional business admin access)
 * Redirects to login if unauthenticated, or to /admin if lacking permission
 * @param {string|string[]} permission - Required permission key(s)
 * @param {Object} options
 * @param {boolean} [options.allowBusinessAdmin=true] - Whether business admins are allowed
 * @returns {Promise<Object>} Authenticated user object
 */
export async function requireAdminWithPermission(permission, options = {}) {
  const { allowBusinessAdmin = true } = options;
  const user = await getUserFromCookies();
  ...
  if (allowBusinessAdmin) {
    allowedRoles.add('business_admin');
  }
  ...
  if (permission) {
    ...
    const hasRequiredPermissions =
      ...
      permissionsToCheck.every((perm) => hasPermission(user, perm));
