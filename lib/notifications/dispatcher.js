...
async function deliverAudience({ roles, tags, userIds }, payload) {
  console.log('DISPATCHER: deliverAudience called', { roles, tags, userIds: userIds.length });
  const deliveries = [];
  const roleTargets = [...roles];

  try {
    if (roleTargets.includes('all')) {
      console.log('DISPATCHER: sending broadcast...');
    ...
    if (roleTargets.length) {
      console.log('DISPATCHER: sending to roles:', roleTargets);
...
export async function sendTemplateNotification({
  ...
  console.log('DISPATCHER: sendTemplateNotification called', { templateType, audienceRoles, audienceTags });
