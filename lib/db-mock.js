...
async function loadFromDisk() {
  try {
    ...
    console.log(`[SAVE] Mock DB loaded ${mockUsers.size} users from disk`);
...
const initMockDb = async () => {
  if (mockUsers.size === 0) {
    ...
    if (!loaded || mockUsers.size === 0) {
      ...
      console.log('[FIX] Mock DB initialized with test users');
      await persistToDisk();
    }
  }
};
...
export async function getDb() {
  if (!mockDbInstance) {
    ...
    console.log('[WARN]  Using MOCK database (MongoDB connection unavailable)');
  }
