...
function status(passed, missing = false) {
  return passed ? 'passed' : missing ? 'missing' : 'failed';
}
...
(async () => {
  ...
  report.checks.push({
    key: 'api.products',
    result: status(p1.ok && Array.isArray(p1.json)),
    evidence: `status=${p1.status}`,
  });
  const p1b = await ping('GET', '/products');
  report.checks.push({
    key: 'ui.products',
    result: status(p1b.status === 200),
    evidence: `status=${p1b.status}`,
  });

  // Stage 2 basic
  const phone = '05' + Math.floor(10000000 + Math.random() * 89999999).toString();
  const reg = await ping('POST', '/api/auth/register', {
  ...
  report.checks.push({
    key: 'auth.register',
    result: status(reg.status === 201),
    evidence: `status=${reg.status}`,
  });
  const login = await ping('POST', '/api/auth/login', { phone, password: '12345678A!' });
  const cookie = login.headers?.['set-cookie'] || '';
  report.checks.push({
    key: 'auth.login',
    result: status(login.status === 200 && cookie.includes('token=')),
    evidence: `status=${login.status}`,
  });
  ...
  report.checks.push({
    key: 'stats.endpoint',
    result: status(s6ok, s6.status === 404),
    evidence: `status=${s6.status}`,
  });
